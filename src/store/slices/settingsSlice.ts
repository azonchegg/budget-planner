...
export const settingsSlice = createSlice({
  ...
  reducers: {
    ...
    // Update currency
    updateCurrency: (state, action: PayloadAction<'USD' | 'EUR' | 'RUB' | 'KZT'>) => {
    ...
    updateCustomCategory: (state, action: PayloadAction<CustomCategory>) => {
      const index = state.settings.customCategories.findIndex((cat) => cat.id === action.payload.id)
      if (index !== -1) {
    ...
    deleteCustomCategory: (state, action: PayloadAction<string>) => {
      state.settings.customCategories = state.settings.customCategories.filter((cat) => cat.id !== action.payload)
    },
