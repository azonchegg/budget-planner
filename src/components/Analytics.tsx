              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={byCategory} dataKey="value" nameKey="name" outerRadius={120} innerRadius={40}>
                      ...
                      <Tooltip
                        formatter={(value: any) =>
                          formatMoney(Number(value), settings.currency as 'USD' | 'EUR' | 'RUB' | 'KZT')
                        }
                      />
                    </PieChart>
                  </ResponsiveContainer>
                  ...
                  {byCategory.map((cat) => (
                    <div key={cat.name} className="flex items-center gap-3 p-3 rounded-lg bg-slate-50">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-600">{((cat.value / total) * 100).toFixed(1)}% of total</p>
                      </div>
                      <p className="font-semibold text-slate-900">
                        {formatMoney(cat.value, settings.currency as 'USD' | 'EUR' | 'RUB' | 'KZT')}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={byDay} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                    ...
                    <XAxis dataKey="date" />
                    <YAxis
                      tickFormatter={(value) => formatMoney(value, settings.currency as 'USD' | 'EUR' | 'RUB' | 'KZT')}
                    />
                    <Tooltip
                      formatter={(value: any) =>
                        formatMoney(Number(value), settings.currency as 'USD' | 'EUR' | 'RUB' | 'KZT')
                  ...
                  <tbody>
                    {filtered.map((expense: any) => (
                      <tr key={expense.id} className="border-t hover:bg-slate-50/60">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-slate-900">{expense.category}</td>
                        <td className="px-6 py-4 text-slate-600">{expense.note || '—'}</td>
                        <td className="px-6 py-4 text-right font-semibold text-slate-900">
                          {formatMoney(expense.amount, settings.currency as 'USD' | 'EUR' | 'RUB' | 'KZT')}
                        </td>
...
function formatMoney(amount: number, currency: 'USD' | 'EUR' | 'RUB' | 'KZT'): string {
